...
{ 
    static char name[20]={""};
    char *q = strrchr(p,'/') + 1;
         strncpy(name,q,strlen(q)-4);//Note the following 6. If the name of the character in your test set (excluding the suffix) is other length, please change the length you need (the official default length is 6). )
    return name;
}
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        int nboxes = 0;
        detection *dets = get_network_boxes(net, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes);
        // printf("The detected class is %d\n", dets->classes);
        // printf("The prob is %f\n",*dets->prob);
        // printf("The x,y,w,h is %f, %f, %f, %f", dets->bbox.x, dets->bbox.y, dets->bbox.w, dets->bbox.h);
        //printf("%d\n", nboxes);
        //if (nms) do_nms_obj(boxes, probs, l.w*l.h*l.n, l.classes, nms);
        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
